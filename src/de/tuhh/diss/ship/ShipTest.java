...
public class ShipTest {
    private static final String SHIP_NAME = "Bella Luisa";
	...
	public static void main(String[] args) {
		// Create a new ship
		Ship myShip = new Ship(SHIP_NAME, PETROL_CONSUMPTION, SIZE_OF_TANK, SPEED);

		// Test if all attributes of the new ship are cleared
		if (!test(1, myShip.getTankFillLevel(), 0, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		...
		if (!test(4, myShip.fillTank(2 * SIZE_OF_TANK), SIZE_OF_TANK, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		if (!test(5, myShip.getTankFillLevel(), SIZE_OF_TANK, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		...
		if (!test(6, myShip.fillTank(SIZE_OF_TANK), 0, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		if (!test(7, myShip.getTankFillLevel(), SIZE_OF_TANK, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		...
		if (!test(9, myShip.getTankFillLevel(), 0, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)){
		    return;  
		}
		...
		if (!test(12, myShip.fillTank(SIZE_OF_TANK), SIZE_OF_TANK, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		if (!test(13, myShip.fillTank(SIZE_OF_TANK / 2), 0, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		
		// Check that the tank is now full, but not more than full. 
		if (!test(14, myShip.getTankFillLevel(), SIZE_OF_TANK, TEST_TOLERANCE_DIGITS_FUEL, UNIT_FUEL)) {
			return;
		}
		...
		// Check the fill level and distance, if they are calculated correctly
		if (!test(17, myShip.getTankFillLevel(), 0, TEST_TOLERANCE_DIGITS_DISTANCE_COUNTER, UNIT_DISTANCE_COUNTERS)) {
			return;
		}
